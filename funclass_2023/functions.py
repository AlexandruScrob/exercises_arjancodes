...
ROOT = Path().absolute()
DATA = ROOT / "funclass_2023" / "data"

FILENAME = "survey_results_public.csv"


def cumulative_count(
    column_name: str, _file: DictReader, field_sep: str | None = None
) -> Counter[str]:
...
def analyze_frequencies(
    ...
    target = cumulative_count(column_name=column_name, _file=_file, field_sep=field_sep)
...
def main() -> None:
    with open(DATA / FILENAME, "r", encoding="utf-8") as csv_file:
        ...
        analyze_frequencies(column_name="RemoteWork", _file=csv_reader)

        # the analysis should run again starting from the first line, not from header.
        csv_file.seek(1)

        analyze_frequencies(
            column_name="LanguageHaveWorkedWith", _file=csv_reader, field_sep=";"
        )
