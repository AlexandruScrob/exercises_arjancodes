...
PATIENT_DB = {
    42: Patient(
        id=42, name="John Doe", vitals={"heart_rate": 72, "blood_pressure": "120/80"}
    ),
    43: Patient(
        id=43, name="Jane Smith", vitals={"heart_rate": 65, "blood_pressure": "110/70"}
    ),
}
...
class TriageOutput(BaseModel):
    ...
    escalate: bool = Field(description="Whether to escalate the case to a human nurse")
...
triage_agent = Agent(
    ...
    system_prompt=(
        ...
        "Always mention the patient's name when available."
    ),
)
...
async def main() -> None:
    deps = TriageDependencies(patient_id=42, db=DatabaseConn())
