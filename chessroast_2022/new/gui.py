...
class GUI(Frame):
    def __init__(self, master, size=85, color1="#eeeed2", color2="#769656"):
        self.rows = 8
        self.columns = 8
        self.size = size
        self.color1 = color1
    ...
    def commandHandler(self, pos: tuple):
        ...
        if isinstance(self.board[self.pieceTo[0][1]][self.pieceTo[0][0]], King):
            if (
                isinstance(self.board[y][x], Rook)
        ...
        if self.game.player_turn == self.board[y][x].color:
            ...
            self.board[self.pieceTo[0][1]][self.pieceTo[0][0]].get_moves(self.board),
            list,
        ):
            if pos in self.board[self.pieceTo[0][1]][self.pieceTo[0][0]].get_moves(
                self.board
            ) and self.pieceTo[0] != (-1, -1):
                self.pieceTo[1] = pos

                # En passant
                if (
                    isinstance(self.board[self.pieceTo[0][1]][self.pieceTo[0][0]], Pawn)
                    and isinstance(self.board[y][x], Empty)
                    and x != self.pieceTo[0][0]
                ):
        ...
        for row in range(self.rows):
            ...
            for col in range(self.columns):
                ...
                self.squareButton[squarenumber] = Button(
                    self.top,
                    command=(lambda pos=(col, row): self.commandHandler(pos)),
                    width=self.size,
                    height=self.size,
                    bg=color,
    ...
    def refresh(self, pos: list):
        for p in pos:
            ...
            x, y = p
            self.images[squareNo] = PhotoImage(
                file=self.game.board[y][x].image
            ).subsample(2, 2)

            self.squareButton[squareNo] = Button(
                self.top,
