...
class Product:
    ...
    def __post_init__(self) -> None:
        if self.unit_price < 0:
            raise ValueError("unit_price attribute must be greater than zero.")

        if self.shipping_weight < 0:
            raise ValueError("shipping_weight attribute must be greater than zero.")
...
class Order:
    ...
    @property
    def total_shipping_weight(self) -> int:
...
def main() -> None:
    ...
    print(f"Value paid in taxes: ${order.tax:.2f}")
