...
def fetch_blog(blog_id) -> Result["Blog", Exception]:
    return flow(blog_id, fetch_blog_from_db, bind(blog_to_dict), bind(verify_access))


# TODO returns a container with a success track and a failure track
@safe
...
def verify_access(blog) -> "Blog":
    ...
    if not blog_public:
        raise NotAuthorizedError(
            f"You are not allowed to access blog with id {blog_id}."
        )
    return blog
